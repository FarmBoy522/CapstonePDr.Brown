...
# delay = 0 s
# auto Trigger = off?
status["trigger"] = ps.ps4000aSetSimpleTrigger(chandle, 1, 0, 1024, 2, 100000 , 0)
assert_pico_ok(status["trigger"])

# Set number of pre and post trigger samples to be collected
preTriggerSamples = 50
postTriggerSamples = 4950
...
for i in range(50):
    ...
    print(reslist[0])
    print(adc2mVChBMax[0])
    for i in range(0, len(adc2mVChBMax)):
        reslist[i]=(reslist[i] + adc2mVChBMax[i])
    
    

    if filenumber == 49:
        #for i in range(0, len(reslist)):
            #reslist[i]=(reslist[i]/50)
        plt.plot(time, reslist[:])
        plt.xlabel('Time (ns)')
        plt.ylabel('Voltage (mV)')
        filename= "test" + str(filenumber)
        plt.savefig('/Users/leyto/OneDrive/Documents/Waveforms/'+ filename)
        print(len(adc2mVChBMax))
